...
function resolveEnRoot(repoRoot, explicit) {
  const candidates = [
    explicit,
    process.env.EN_ROOT,
    path.join(repoRoot, '.upstream', 'source', 'scenes'),
    // 本地布局 A:raspd/origin/<game>/source/scenes(仓库位于 raspd/zh/<repo>)
    path.resolve(repoRoot, '..', '..', '..', 'origin', 'social_democracy_redux', 'source', 'scenes'),
  ...
  if (!found) {
    ...
    console.error('  本地:确保存在 raspd/origin/social_democracy_redux/source/scenes');
